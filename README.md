...
**Global server management:** Use `--global` with `list`, `stop`, and `stop-all` to operate across all projects system-wide. `server list --global` shows all running ClickHouse servers with a Project column indicating which directory each belongs to. The PID column is the process to signal to stop a server for good, which is the ClickHouse watchdog when the server has one (killing the supervised server process only makes the watchdog restart it). Both `server list` and `server list --global` report that same PID for a given server.
