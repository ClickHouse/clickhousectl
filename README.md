...
The new binding also takes a moment to reach the Query API host, and a query issued in between is rejected. First-use provisioning already waits for the endpoint to accept the new key before running the statement; `repair-query-key` does the same on a running service, probing the endpoint with the new key (`SELECT 1`) and exiting 0 only once it is accepted. An idle or stopped service is not probed, because the probe would wake the one and the other cannot answer: the replacement is stored and a note says the next query verifies it. If the probe fails for a reason other than the key not being ready yet, the repair is still complete and is reported as such, with the probe's failure alongside; nothing is rolled back.
