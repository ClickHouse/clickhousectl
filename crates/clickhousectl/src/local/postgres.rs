...
enum RollbackMetadata<'a> {
    Locked(&'a server::ServerLock),
    #[cfg(test)]
    Path(PathBuf),
}

impl RollbackMetadata<'_> {
    fn path(&self) -> PathBuf {
        match self {
            Self::Locked(lock) => lock.metadata_path(),
            #[cfg(test)]
    ...
    fn remove(&self) -> Result<()> {
        match self {
            Self::Locked(lock) => lock.remove_info(),
            #[cfg(test)]
            Self::Path(path) => match std::fs::remove_file(path) {
                Ok(()) => Ok(()),
    ...
    fn save(&self, info: &ServerInfo) -> Result<()> {
        match self {
            Self::Locked(lock) => lock.save_info(info),
            #[cfg(test)]
            Self::Path(path) => {
...
struct FreshStartRollback<'a> {
    container_id: String,
    info: ServerInfo,
    metadata: RollbackMetadata<'a>,
...
async fn rollback_fresh_start(
    ...
    if container_removed && instance_removed {
        match rollback.metadata.remove() {
            Ok(()) => return diagnostics,
            Err(error) => diagnostics.push(format!(
                "failed to remove metadata '{}': {error}",
                rollback.metadata.path().display()
            )),
        }
    } else {
        ...
        match rollback.metadata.save(&rollback.info) {
...
fn default_pg_name() -> Result<String> {
...
async fn resume_existing(
    docker: &bollard::Docker,
    prior: ServerInfo,
    metadata_lock: &server::ServerLock,
    json: bool,
) -> Result<()> {
...
fn remove(name: &str, version: Option<&str>, json: bool) -> Result<()> {
    ...
    let mut target = resolve_pg_target(name, version)?;
    let key = target.name.clone();
    let metadata_lock = server::ServerLock::acquire(&key)?;
    target = metadata_lock
        .load_info()?
        .filter(|info| info.engine == Engine::Postgres)
...
async fn client(
    ...
    let target = resolve_pg_target(server_name, version.as_deref())?;
    let metadata_lock = server::ServerLock::acquire(&target.name)?;
    let info = metadata_lock
        .load_info()?
        .filter(|info| info.engine == Engine::Postgres)
...
fn dotenv(name: Option<&str>, version: Option<&str>, use_local: bool, json: bool) -> Result<()> {
    ...
    let target = resolve_pg_target(server_name, version)?;
    let metadata_lock = server::ServerLock::acquire(&target.name)?;
    let info = metadata_lock
        .load_info()?
        .filter(|info| info.engine == Engine::Postgres)
    ...
    async fn fresh_initialization_timeout_removes_container_metadata_and_pgdata() {
        ...
        let error = finish_fresh_start(
            ...
            FreshStartRollback {
                ...
                metadata: RollbackMetadata::Path(metadata_path.clone()),
