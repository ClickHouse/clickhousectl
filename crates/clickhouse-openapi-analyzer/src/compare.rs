...
mod tests {
    ...
    #[test]
    fn helper_exemptions_become_stale_when_removed_or_added_to_openapi() {
        let config = AnalyzerConfig {
            non_openapi_client_methods: BTreeSet::from(["helper".into()]),
            ..AnalyzerConfig::default()
        };
        for (client, paths, stale) in [
            (
                "pub struct Client; impl Client { pub async fn helper(&self) {} }",
                serde_json::json!({}),
                false,
            ),
            (
            ...
            (
                "pub struct Client; impl Client { pub async fn helper(&self) {} }",
                serde_json::json!({"/helper": {"get": {"operationId": "helper"}}}),
                true,
            ),
        ] {
            let rust = RustInventory::parse(client, "", "").unwrap();
    ...
    #[test]
    fn partial_required_exemptions_require_an_effective_request_override() {
        let config = AnalyzerConfig {
        ...
        let mut document = serde_json::json!({
            "paths": {}, "components": {"schemas": {"Widget": {
                "type": "object", "properties": {"name": {"type": "string"}, "note": {"type": "string", "description": "Optional note"}}, "required": []
            }}}
        });
        let rust = RustInventory::parse(
            "",
            "pub struct Widget { pub name: String, pub note: Option<String> }",
            "",
        )
        .unwrap();
        let check = |document: &serde_json::Value| {
            let spec = OpenApiInventory::build(document, &config).unwrap();
            compare(&rust, &spec, &spec, &config)
                .findings
                .iter()
                .any(|f| f.kind == FindingKind::StaleExemption)
        };
        assert!(!check(&document));
        document["components"]["schemas"]["Widget"]["required"] = serde_json::json!(["name"]);
        assert!(check(&document), "upstream completed required[]");
        ...
        assert!(
            check(&document),
            "no explicit array: override is ineffective"
        );
        document["components"]["schemas"]["Widget"]["required"] = serde_json::json!([]);
        document["paths"] = serde_json::json!({"/widgets": {"get": {"operationId": "getWidgets", "responses": {
            "200": {"content": {"application/json": {"schema": {"$ref": "#/components/schemas/Widget"}}}}
        }}}});
        assert!(
            check(&document),
            "response-only requiredness must not keep an override alive"
        );
        document["components"]["schemas"] = serde_json::json!({});
        assert!(check(&document), "deleted schema");
    }

    #[test]
    fn acknowledged_constraints_still_report_changed_values() {
        let pointer = "/components/schemas/Widget/properties/mode";
        let config = AnalyzerConfig {
            acknowledged_unsupported_enum_pointers: BTreeSet::from([pointer.into()]),
            ..AnalyzerConfig::default()
        };
        let rust = RustInventory::parse("", "pub struct Widget { pub mode: String }", "").unwrap();
        for (before, after, changed) in [
            ...
            let make_spec = |values| {
                OpenApiInventory::build(&serde_json::json!({
                "paths": {}, "components": {"schemas": {"Widget": {"properties": {"mode": {"enum": values}}}}}
            }), &config).unwrap()
            };
            let snapshot = make_spec(before);
            let spec = make_spec(after);
            let report = compare(&rust, &spec, &snapshot, &config);
            assert_eq!(
                report
                    .findings
                    .iter()
                    .any(|f| f.kind == FindingKind::AcknowledgedEnumConstraintChanged),
                changed
            );
